  ...
  section.bg-black.text-white.pt-20
    ...
    .content-container.text-center.pb-14
      hgroup
        ...
        h2.mx-auto.max-w-xl.text-xl.text-gray-300.mb-12 Easily utilize the latest AI to create professional, original images from your Hubspot content.
    ...
    //- Abilities
    .content-container.grid.md_grid-cols-2.lg_grid-cols-3.gap-x-20.gap-y-12.md_gap-y-16.py-16
      +each('pricingOptions as option')
        +if('option.link')
          a(href='{option.link}').ability
            h2 {option.option === 'Image Creation' ? 'Infinite Image Creation' : option.option}
            p.pr-8 {option.note}
